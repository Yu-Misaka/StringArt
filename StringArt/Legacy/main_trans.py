import numpy as np
from PIL import Image, ImageDraw, ImageOps # ImageOps needed for equalize
import math
import argparse
...
IMG_SIZE = 500
DEFAULT_N_PINS = 288
DEFAULT_MAX_LINES = 4000 # Might be reducible with new techniques
MIN_DISTANCE = 20
MIN_LOOP = 20
DEFAULT_LINE_WEIGHT = 25 # Weight for SUBTRACTING from error map
SCALE = 15
# --- New Parameters ---
DEFAULT_ERROR_POWER = 1.5 # Power for error calculation (1.0 = linear, >1.0 emphasizes darker areas more)
DEFAULT_LINE_BRIGHTNESS = 0.08 # Brightness added per line to output accumulator (adjust for desired intensity/transparency)

# --- Helper Functions ---

def load_and_preprocess_image(image_path, target_size, apply_equalization=True): # Added flag
    """Loads, crops, resizes, grayscales, optionally equalizes, and circle-crops."""
    try:
    ...
    # --- Cropping, Resizing, Masking (same as before) ---
...
# --- calculate_pin_coords, get_line_pixels, precalculate_lines (remain the same) ---
...
def generate_string_art(image_array, num_pins, max_lines, min_distance, min_loop,
                        line_weight, error_power, line_brightness, # Added params
    ...
    error_image = 255.0 - image_array # float32, high value = dark area

    # ***** Output Accumulator for Semi-Transparency *****
    ...
    for line_num in range(max_lines):
        best_pin = -1
        max_weighted_error = -np.inf # Maximize the *weighted* error sum
        ...
        for offset in range(min_distance, num_pins - min_distance):
            ...
            # Only consider positive error values for the power calculation
            ...
            if np.any(positive_error_mask):
                 ...
                 weighted_error = 0 # No positive error on this line


            if weighted_error > max_weighted_error:
                max_weighted_error = weighted_error
                best_pin = test_pin
        ...
        # Subtract darkness (weight) from the error image
        error_image[rows, cols] -= line_weight
        np.clip(error_image, 0, None, out=error_image) # Clip lower bound to 0

        # ***** Add Brightness to Accumulator *****
        # Get line pixels on the *scaled* output grid
        ...
        # Add the defined brightness for this line
    ...
    # --- Finalize Output Image from Accumulator ---
    print("Normalizing accumulator and creating final image...")
    # Normalize the accumulator: scale values to 0-255
    # Option 1: Simple max scaling (can make images with few overlaps dim)
    # max_brightness = np.max(output_accumulator)
    # if max_brightness > 0:
    #     normalized_output = (output_accumulator / max_brightness) * 255.0
    # else:
    #     normalized_output = np.zeros_like(output_accumulator)

    # Option 2: Clip at a certain percentile or value before scaling (often looks better)
    # Clip accumulator values to prevent extreme brightness from single pixels dominating scaling
    # Let's clip based on a percentile of the *non-zero* values
    non_zero_accumulator = output_accumulator[output_accumulator > 0]
    if non_zero_accumulator.size > 0:
        # Use a high percentile (e.g., 99th) as the effective "maximum" brightness
        # Adjust percentile (e.g., 98, 99.5) or use a fixed clip_value if needed
        clip_value = np.percentile(non_zero_accumulator, 99.0)
        # Ensure clip_value is at least a minimum brightness to avoid division by zero or near-zero
        clip_value = max(clip_value, line_brightness * 2) # At least brightness of 2 overlapping lines
        
        # Clip and scale
        clipped_accumulator = np.clip(output_accumulator, 0, clip_value)
        normalized_output = (clipped_accumulator / clip_value) * 255.0
    else:
        # If accumulator is all zeros
        normalized_output = np.zeros_like(output_accumulator)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--output_png", default="string_art_preview_accum.png", help="Output file for the preview image.") # New default name
    # --- Existing Args ---
    ...
    parser.add_argument("--error_power", type=float, default=DEFAULT_ERROR_POWER, help=f"Exponent for error calculation (1=linear, >1 emphasizes dark areas; default: {DEFAULT_ERROR_POWER})")
    parser.add_argument("--brightness", type=float, default=DEFAULT_LINE_BRIGHTNESS, help=f"Brightness added per line to output (controls transparency effect; default: {DEFAULT_LINE_BRIGHTNESS})")
    parser.add_argument("--previewpins", action='store_true', help="Draw markers for pins on the preview image.")

    args = parser.parse_args()

    # Validation
    # ... (add validation for new args if needed, e.g., error_power > 0, brightness > 0)
    ...
    # 4. Generate String Art Sequence & Image
    sequence, preview_image = generate_string_art(
        ...
        args.minloop,
        args.weight,
        args.error_power, # Pass new arg
        args.brightness,  # Pass new arg
        pin_coordinates,
        line_pixel_cache
    )
